...
window.addEventListener('DOMContentLoaded', function(){
		...
		TERIABLE.NoiseStackItem = '<item><select></select><action act="accept-item">Accept</action></item>';
			...
			'<select id="perlin-2d-mode" value="Absolute"><option>Absolute</option><option>Additive</option><option>Subtractive</option></select><BR />'+
		...
		TERIABLE.ApplyNoiseStack = function(count){
			if(count<$('#NoiseStack item').length){
				
				TERIABLE.ApplyItem(count,0);
				/*this._block = TERIABLE.Region[count];
				TERIABLE.UpdatePositions(this._block);*/
			count++;
				return	setTimeout(function(){TERIABLE.ApplyNoiseStack(count)},0);
			}
			return;
		}
